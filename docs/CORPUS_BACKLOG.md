...
**Última actualización:** 2026-05-20 (novena tanda — alcaldías mayores y gobernaciones; 68→100 queries humanas en curso) — 4 bugs nuevos relacionados con resolución de entidades complejas. ENTITY-MISRESOLUTION-FALLBACK-001 confirmado en `rewrite_gobernacion` también (Bolívar→ICBF, Medellín→INDER). Decisión arquitectónica ENTITY-ECOSYSTEM-EXPANSION aplazada a Sprint 3 con interfaz preparada en Sprint 1A. Total acumulado: **32 categorías de bugs, 9 CRITICAL** (sin cambio en CRITICAL count). Plan de implementación detallado para Codex registrado en [docs/SPRINT_1A_IMPLEMENTATION_PLAN.md](SPRINT_1A_IMPLEMENTATION_PLAN.md).
...
| ENTITY-MISRESOLUTION-FALLBACK-001 | `rewrite_alcald[ií]a` Y `rewrite_gobernaci[oó]n` devuelven entidad arbitraria con `confidence=high` cuando no encuentran match. **Confirmados:** Paipa→Manizales, Medellín→INDER (alcaldía), Bolívar→ICBF (gobernación). Patrón sistémico en ambas funciones de reescritura | **3 verificados** (2 alcaldía, 1 gobernación) | CRITICAL — bug compartido entre dos funciones, fix una vez sirve para ambas |
...
| FOLLOWUP-ENTITY-PURGE-001 | Follow-up que cambia scope territorial (depto → ciudad) no purga la entidad heredada mal resuelta. Caso: gobernación Bolívar (mal resuelta a ICBF) → "magangue" → mantuvo ICBF + agregó "magangue" como objeto contractual | 1 verificado | **HIGH (nuevo)** — extension de FOLLOWUP-DATASET-PURGE-001 a campo entidad |
| MORPHO-EXPANSION-INCONSISTENT-001 | Expansión morfológica de sinónimos se aplica en algunas rutas (heuristic_only) pero no en otras (heuristic_plus_llm). Caso adulto mayor: con LLM solo buscó "adulto mayor" sin expandir a "personas mayores", "centros de vida", etc. | 1 verificado | **MEDIUM (nuevo)** |
| ENTITY-ECOSYSTEM-EXPANSION-001 | Alcaldías mayores y gobernaciones grandes (Bogotá, Medellín, Cali, Valle, Antioquia, etc.) tienen 30-60+ entidades dependientes. Modelo actual solo busca entidad central. Pregunta abierta: ¿strict, ecosistema, híbrido con sugerencia? | observación arquitectónica | **HIGH (nuevo) — APLAZADO A SPRINT 3** con interfaz preparada en Sprint 1A |
...
| COURTESY-FILLER-001 | `hola`, `estoy interesado`, `actualmente`, `algún`, vocabulario de queja (`nada`, `terrible`, `incorrecto`, `mal`, `fracaso`), señales de bidder (`oferta` como objeto cuando significa "oportunidad"), y verbos interrogativos (`fueron`, `fue`, `son`, `están`) entran como objeto contractual | 4 | HIGH |
